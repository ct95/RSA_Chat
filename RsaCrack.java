...
//    }
    ...
    public void crackFunction(){
        ...
        while(true){
            System.out.println("Enter char(s) to decrypt, or type quit to quit");
            String strToDecrypt = scanIn.nextLine();
            if(strToDecrypt.charAt(0) == 'q' || strToDecrypt.charAt(0) == 'Q') break;
            String [] parsedStrToDecrypt = strToDecrypt.split(" ");
            int [] parsedIntToDecrypt = new int[parsedStrToDecrypt.length];
            StringBuilder sb = new StringBuilder();
            for(int i=0; i<parsedStrToDecrypt.length; i++){
                parsedIntToDecrypt[i] = Integer.parseInt(parsedStrToDecrypt[i]);
                int decryptedInt = this.endecrypt(parsedIntToDecrypt[i], rtn[2], c);
                char decryptedChar = (char)decryptedInt;
                sb.append(decryptedChar);
            }
            System.out.println("Input decrypted as >> " + sb.toString());


//            int toDecrypt = Integer.parseInt(strToDecrypt);
//            int decryptedInt = this.endecrypt(toDecrypt, rtn[2], c);
//            char decryptedChar = (char)decryptedInt;
...
//            System.out.println("The letter is " + decryptedChar);
    ...
    public static void main(String[] args) {
        RsaCrack rsaCrack = new RsaCrack();

        rsaCrack.crackFunction();
